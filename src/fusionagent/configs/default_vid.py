...
_C.DATA.ROOT = '/research/cvl-zhujie4/data'
# Dataset for evaluation
_C.DATA.DATASET = 'mevid'
# Whether split each full-length video in the training set into some clips
_C.DATA.DENSE_SAMPLING = True # True for CCVID, MEVID
# Sampling step of dense sampling for training set (i.e. seq_len) if DENSE_SAMPLING is True
_C.DATA.SAMPLING_STEP = 8
...
_C.DATA.TRAIN_BATCH = 2
# Batch size for testing
_C.DATA.TEST_BATCH = 1 # 1 since length of facial data is variable
...
# Sequence length of each input video clip if using temporal transform also (for query and gallery)
# seq_len = 8 for our test evaluation
_C.AUG.SEQ_LEN = 8
# Sampling stride of each input video clip if using temporal transform
...
_C.OUTPUT = '/home/jiezhu/Desktop/code/cvpr-lsf'
...
def update_config(config, args):
    ...
    if hasattr(args, 'train_batch'):
        config.DATA.TRAIN_BATCH = args.train_batch
    if hasattr(args, 'num_sample'):
        config.DATA.SAMPLING_STEP = args.num_sample
    if hasattr(args, 'num_worker'):
        config.DATA.NUM_WORKERS = args.num_worker
    ...
    if args is not None:
        ...
        for key, value in args_dict.items():
            if not hasattr(config, key) and key not in ['cfg', 'root', 'output', 'resume', 'eval', 'tag', 'dataset', 'gpu', 'amp']:
