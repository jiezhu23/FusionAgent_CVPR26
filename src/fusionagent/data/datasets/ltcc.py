...
class LTCC(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        few_shot = kwargs.get('few_shot', None)
        if few_shot is not None:
            print(f"Note: Using {few_shot} shots for training set!")
        train, num_train_pids, num_train_imgs, num_train_clothes, pid2clothes = \
            self._process_dir_train(self.train_dir, few_shot=few_shot)
    ...
    def _process_dir_train(self, dir_path, few_shot=None):
        if hasattr(self, 'body_data'):
        ...
        if few_shot is not None:
            ...
            new_img_paths = []
            for pid in sorted(pid_to_images.keys()):
                samples = pid_to_images[pid][:min(few_shot, len(pid_to_images[pid]))]
                new_img_paths.extend(samples)
