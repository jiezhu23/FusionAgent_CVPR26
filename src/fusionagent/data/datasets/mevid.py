...
from utils.file_utils import mkdir_if_missing, write_json, read_json

class MEVID(object):
    """ MEVID
    Credit from: https://github.com/Kitware/MEVID/blob/main/CAL/data/datasets/ccvid.py
    """
    def __init__(self, root='../../mevid/', sampling_step=64, seq_len=16, stride=4, **kwargs):
        self.root = osp.join(root, 'mevid')
        ...
        train_info = np.loadtxt(self.train_info_path).astype(int)
        test_names = self._get_names(self.test_name_path)
        test_info = np.loadtxt(self.test_info_path).astype(int)
        query_IDX = np.loadtxt(self.query_path).astype(int)
       ...
       # reformat the data
        train = {'dataset': train, 'dataset_name':'mevid'}
        train_dense = {'dataset': train_dense, 'dataset_name':'mevid'}
        query = {'dataset': query}
        gallery = {'dataset': gallery}
        recombined_query = {'dataset': recombined_query}
        recombined_gallery = {'dataset': recombined_gallery}
        if hasattr(self, 'body_data'):
            train['body_data'] = self.body_data
            train_dense['body_data'] = self.body_data
            query['body_data'] = self.body_data
            gallery['body_data'] = self.body_data
            recombined_query['body_data'] = self.body_data
            recombined_gallery['body_data'] = self.body_data
        if hasattr(self, 'face_data'):
            train['face_data'] = self.face_data
            train_dense['face_data'] = self.face_data
            query['face_data'] = self.face_data
            gallery['face_data'] = self.face_data
            recombined_query['face_data'] = self.face_data
            recombined_gallery['face_data'] = self.face_data
    ...
    def _check_before_run(self):
        ...
        if osp.exists(osp.join(self.root, 'mevid.h5')):
            self.body_data = h5py.File(osp.join(self.root, 'mevid.h5'), 'r')
            print('h5 format file detected, using h5 format data')
            if not osp.exists(osp.join(self.root, 'mevid_face_cleaned.h5')):
                print('face data is not available, using body data only')
            else:
                self.face_data = h5py.File(osp.join(self.root, 'mevid_face_cleaned.h5'), 'r')
    ...
    def _process_mevid_data(self, track_names, track_info, home_dir, relabel=False, clothes2label=None):
        ...
        for sta_idx, end_idx, pid, clothes_label, camid in tracklet_path_list:
            ...
            if len(img_names) == 0:
                continue
            if hasattr(self, 'body_data'):
                img_paths = [osp.join(home_dir, img_name[:4], img_name[:-4]) for img_name in img_names] # h5 file format
            else:            
                img_paths = [osp.join(self.root, home_dir, img_name[:4], img_name) for img_name in img_names]  # original

            clothes = '{}_{}'.format(pid, clothes_label)
            clothes_id = clothes2label[clothes]
            pid2clothes[pid2label[pid], clothes_id] = 1
            
            pid_face_dict = {pid: [] for pid in pid_container}
