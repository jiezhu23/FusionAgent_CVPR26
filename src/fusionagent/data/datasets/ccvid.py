...
class CCVID(object):
    ...
    def __init__(self, root='/data/datasets/', sampling_step=64, seq_len=16, stride=4, **kwargs):
        ...
        # reformat the data
        train = {'dataset': train, 'dataset_name':'ccvid'}
        train_dense = {'dataset': train_dense, 'dataset_name':'ccvid'}
        query = {'dataset': query}
        ...
        logger.info("  train_dense  | {:5d} | {:11d} | {:9d}".format(num_train_pids, len(train_dense['dataset']), num_train_clothes))
    ...
    def _check_before_run(self):
        ...
        if osp.exists(osp.join(self.root, 'ccvid.h5')):
            self.body_data = h5py.File(osp.join(self.root, 'ccvid.h5'), 'r')
            print('h5 format file detected, using h5 format data')
            if not osp.exists(osp.join(self.root, 'ccvid_face.h5')):
                print('face data is not available, using body data only')
            else:
                self.face_data = h5py.File(osp.join(self.root, 'ccvid_face.h5'), 'r')
    ...
    def _process_data(self, data_path, relabel=False, clothes2label=None):
        ...
        for tracklet_path, pid, clothes_label in tracklet_path_list:
            if hasattr(self, 'body_data'):
                img_paths = [osp.join(tracklet_path,idx) for idx in self.body_data[tracklet_path].keys()] # h5 file format
            else:
                img_paths = glob.glob(osp.join(self.root, tracklet_path, '*')) # original
                ...
                if num_sampling == 0:
                    ...
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            # discard the remained frames if it's length is less than sampling_step
