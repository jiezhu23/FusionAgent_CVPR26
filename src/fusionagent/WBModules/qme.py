...
class QME(nn.Module):
    ...
    def get_face_feat(self, face=None, mode='kprpe', aggregated=True):
        ...
        if 'adaface' in mode:
            ...
            with torch.no_grad():
                ...
                face = einops.rearrange(face, 'b c t h w -> (b t) c h w')
                # get face landmarks
                aligned_x, orig_ldmks, aligned_ldmks, score, thetas, bbox = self.face_model[mode].aligner(face)
                keypoints = orig_ldmks  # torch.randn(1, 5, 2)
                face = self.face_model[mode](face, keypoints)
                face = einops.rearrange(face, '(b t) d  -> b t d', b=bs)  # (B, N, 512)
            for idx in [8, 16]:
                intermediate_feats.append(self.face_model[mode].model.net.blocks[idx].hook_output) # (B * T, num_patches, c)
            intermediate_feats = partial(nn.LayerNorm, eps=1e-6)(intermediate_feats[0].shape[-1],
    ...
    def get_vlm_feat(self, body=None, aggregated=True, mode='qwen'):
        # input dim: (B, C, T, 256, 128)
        # output dim: vlm:(B, N, 512), intermediate_feats: None
        if body is None or body.numel()==0:
            return None, None
        bs, c, t, h, w = body.shape
        if 'qwen2.5' in mode:
            with torch.no_grad():
                body = einops.rearrange(body, 'b c t h w -> (b t) c h w')
                vision_tower_output = self.backbone_dict[mode].vision_tower(body)
    ...
    def get_feats(self, inputs, aggregated=True):
        ...
        for model_name in self.model_list:
            if 'kprpe' in model_name or 'adaface' in model_name or 'arcface' in model_name:
                ...
                vlm_feat, _ = self.get_vlm_feat(inputs[model_name], aggregated=aggregated, mode=model_name)
                feats_list[model_name] = vlm_feat
                interm_feats_list[model_name] = None
            else:
...
if __name__ == '__main__':
    ...
    # f = h5py.File('/data/CCVID/ccvid_face.h5', 'r')
    # clip = [Image.fromarray(f['session1/001_01/00001']['face_0'][:])]
    # # clip = 
    ...
    #         ])
    # clip = np.array([test_transform(img) for img in clip]) # (n, 3, 112, 112)
    # feats = model.session.run(model.output_names, {model.input_name: clip})[0] # (1, 512)
