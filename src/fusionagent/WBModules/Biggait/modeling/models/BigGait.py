...
from .DINOv2 import vit_small, vit_large
# import GaitBase & DINOv2_small
from .GaitBase import Baseline
from .save_img import pca_image
...
def padding_resize(x, ratios, target_h, target_w):
    ...
    x = torch.concat([
        F.pad(F.interpolate(x[i:i + 1, ...], (h, need_w[i]), mode="bilinear", align_corners=False),
        (pad_left[i], pad_right[i])) if need_padding_mask[i] else F.interpolate(
            x[i:i + 1, ...], (h, need_w[i]), mode="bilinear", align_corners=False)[..., pad_left[i]:pad_left[i] + w]
...
class BigGait__Dinov2_Gaitbase(nn.Module):
    def __init__(self, model_cfg):
        super(BigGait__Dinov2_Gaitbase, self).__init__()
    ...
    def init_Mask_Branch(self):
        print(f'load model from: {self.pretrained_mask_branch}')
        ...
        print('Missing keys: {}'.format(msg.missing_keys))
        print('Unexpected keys: {}'.format(msg.unexpected_keys))
        print(f"=> loaded successfully '{self.pretrained_mask_branch}'")
        print('SegmentationBranch Count: {:.5f}M'.format(n_parameters / 1e6))
    ...
    def init_parameters(self):
        ...
        print('Expect backbone Count: {:.5f}M'.format(n_parameters / 1e6))
        ...
        n_parameters = sum(p.numel() for p in self.parameters())
        print('All Backbone Count: {:.5f}M'.format(n_parameters / 1e6))
    ...
    def connect_loss(self, images, n, s, c):
        ...
        gradient_x = F.conv2d(images,
                              torch.Tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]])[None, None, ...].repeat(1, c, 1, 1).to(
                                  images.dtype).to(images.device), padding=1)
        gradient_y = F.conv2d(images,
                              torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]])[None, None, ...].repeat(1, c, 1, 1).to(
                                  images.dtype).to(images.device), padding=1)
        loss_connectivity = (torch.sum(torch.abs(gradient_x)) + torch.sum(torch.abs(gradient_y))) / (
                n * s * c * self.sils_size * 2 * self.sils_size)
    ...
    def forward(self, inputs):
        ...
        # ratios = ipts[1]  # real_image_ratios     shape: [n,s,ratio];     ratio: h/w,  e.g. 224/112=0.5;
        del ipts
        ...
        with torch.no_grad():
            ...
            # outs = self.preprocess(ratio_resize(sils, ratios, 256, 128), self.image_size)         # [ns,c,448,224]    if have not used pad_resize for input images
            ...
            outs_last1 = rearrange(outs_last1.view(n, s, self.image_size // 7, self.image_size // 14, -1),
                                   'n s h w c -> (n s) c h w').contiguous()
            outs_last4 = rearrange(outs_last4.view(n, s, self.image_size // 7, self.image_size // 14, -1),
                                   'n s h w c -> (n s) c h w').contiguous()
            ...
            outs_last1 = rearrange(outs_last1.view(n, s, -1, self.sils_size * 2, self.sils_size),
                                   'n s c h w -> (n s) (h w) c').contiguous()
            outs_last4 = rearrange(outs_last4.view(n, s, -1, self.sils_size * 2, self.sils_size),
                                   'n s c h w -> (n s) (h w) c').contiguous()

        # get foreground
        # mask = torch.ones_like(
        #     outs_last1[..., 0], device=outs_last1.device, dtype=outs_last1.dtype
        # ).view(n * s, 1, self.sils_size * 2, self.sils_size)
        # mask = padding_resize(mask, ratios, self.sils_size * 2, self.sils_size)
        normalized_black = (- torch.as_tensor([0.485, 0.456, 0.406], device=outs_last1.device) / torch.as_tensor(
            [0.229, 0.224, 0.225], device=outs_last1.device)).view(1, 3, 1, 1)
        mask = ~torch.isclose(sils, normalized_black).all(dim=1, keepdim=True).all(dim=2)  # [ns, 1, w]
        mask = F.interpolate(mask.half(), size=(self.sils_size,), mode='nearest-exact')
        mask = repeat(mask, 'ns 1 w -> ns 1 h w', h=self.sils_size * 2)
        mask = rearrange(mask, 'ns 1 h w -> ns (h w) 1').contiguous()
        ...
        foreground = torch.zeros_like(
            mask, dtype=fore_feat.dtype, device=fore_feat.device).view(-1, 1).repeat(1, self.mask_dim)
        ...
        foreground = self.get_body(foreground.view(n * s, self.sils_size * 2, self.sils_size, self.mask_dim)).view(
            n * s, -1)  # [n*s,h*w]
        foreground = self.get_edge(
            foreground.view(n * s, 1, self.sils_size * 2, self.sils_size)
        ).view(n * s, -1)  # [n*s,h*w]
        del fore_feat, mask
        ...
        denosing = torch.zeros_like(
            foreground, dtype=den_feat.dtype, device=den_feat.device
        ).view(-1, 1).repeat(1, self.denoising_dim)
        denosing[foreground.view(-1) != 0] = den_feat
        loss_connectivity_part = self.connect_loss(
            denosing.view(n * s, -1, self.denoising_dim)[..., :-1].permute(0, 2, 1), n, s, (self.denoising_dim - 1))
        ...
        appearance = torch.zeros_like(
            foreground, dtype=app_feat.dtype, device=app_feat.device
        ).view(-1, 1).repeat(1, self.app_dim)
        ...
        if self.training:
            try:
                vis_num = min(5, n * s)
                vis_mask = foreground.view(n * s, self.sils_size * 2 * self.sils_size, -1)[
                           :vis_num].detach().cpu().numpy()
                vis_denosing = pca_image(data={
                    'embeddings': denosing.view(n * s, self.sils_size * 2 * self.sils_size, -1)[
                                  :vis_num].detach().cpu().numpy()}, mask=vis_mask, root=None, model_name=None,
                    dataset=None, n_components=3, is_return=True)  # n s c h w
                vis_appearance = pca_image(data={
                    'embeddings': appearance.view(n * s, self.sils_size * 2 * self.sils_size, -1)[
                                  :vis_num].detach().cpu().numpy()}, mask=vis_mask, root=None, model_name=None,
                    dataset=None, n_components=3, is_return=True)  # n s c h w
            except:
                vis_denosing = torch.ones_like(foreground).view(
                    n, s, 1, self.sils_size * 2, self.sils_size).detach().cpu().numpy()
                vis_appearance = torch.ones_like(foreground).view(
                    n, s, 1, self.sils_size * 2, self.sils_size).detach().cpu().numpy()
        ...
        if self.training:
            ...
            for i in mask_idx:
                ...
                for j in idx:
                    feat_list[j][i] = torch.zeros_like(
                        feat_list[j][i], device=feat_list[j].device, dtype=feat_list[j].dtype)
        ...
        if self.training:
            retval = {
                ...
                'visual_summary': {
                    ...
                    'image/foreground': self.min_max_norm(
                        rearrange(foreground.view(n, s, self.sils_size * 2, self.sils_size, -1),
                                  'n s h w c -> (n s) c h w').contiguous()),
                    'image/denosing': self.min_max_norm(
                        rearrange(torch.from_numpy(vis_denosing).float(), 'n s c h w -> (n s) c h w').contiguous()),
                    'image/appearance': self.min_max_norm(
                        rearrange(torch.from_numpy(vis_appearance).float(), 'n s c h w -> (n s) c h w').contiguous()),
                },
                'inference_feat': {
                    'embeddings': embed_1
                }
            ...
            retval = {
                ...
                'intermediate_feats': outs["x_norm_patchtokens_mid4"].contiguous()
            }
        return retval
