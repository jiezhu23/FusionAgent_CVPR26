...
def test_score_wo_openset(score_mat, merge_score_mat, 
               q_pids, q_camids, q_clothes_ids, 
               g_pids, g_camids, g_clothes_ids, unique_g_pids, 
               dataset='ccvid', include_openset=False, log=False, seed=None):
    """
    Args:
        score_mat (np.array): similarity matrix with shape (num_query, num_gallery).
        merge_score_mat (np.array): similarity matrix with shape (num_query, num_gallery).
    ...
    if include_openset:
        ...
        result = pd.concat([gen_res, sc_res, cc_res, verification_result], axis=0)
        if log:
            logger.info("--------------- Evaluation Protocol V1.0.0 -----------------")
            logger.info('Number of Probes: {}, Number of Galleries subjects: {}'.format(int(result['num_probe_templates']), int(result['num_gallery_subjects'])))
            logger.info('Tar@1.00%FAR:{:.1%} Rank1:{:.1%}'.format(result[r'TAR@1.00%FAR'], result['GR_top1']))
            logger.info("------------------------------------------------------------")
    return result



def act_score_fusion(score_mats, merge_score_mats, model_mask, anchor_model_list, top_k=10, norm_method='zscore'):
    """
    Fuse scores from multiple models based on a model mask using proposed Anchor-based Confidence Top-k strategy (ACT).

    Args:
        score_mats (dict): A dictionary of score matrices from different models. 
                           Keys are model names, values are numpy arrays of shape (num_probes, num_gallery).
        merge_score_mats (dict): A dictionary of merged score matrices.
        model_mask (np.ndarray): A boolean array of shape (num_probes, num_models) indicating which models to use for each probe.
        anchor_models_list (list): A list of anchor model names to initialize the fusion for each probe. Length must be num_probes.
        top_k (int): The number of top scores to use from each model for fusion.
        norm_method (str): The normalization method to apply to scores before fusion.

    Returns:
        tuple: A tuple containing:
            - np.ndarray: The fused score matrix.
            - np.ndarray: The fused merged score matrix.
    """
    model_list = list(score_mats.keys())

    # Initialize fused scores with each probe's anchor model's normalized scores
    fused_score_mat = np.zeros(score_mats[model_list[0]].shape)
    fused_merge_score_mat = np.zeros(merge_score_mats[model_list[0]].shape)

    # proposed score-fusion method
    for i in range(score_mats[model_list[0]].shape[0]):
        models_used_mask = model_mask[i]
        num_models_used = models_used_mask.sum()
        if models_used_mask.any():  # Only fuse if at least one model was used
            # Use the anchor model as the initial score
            anchor_model = anchor_model_list[i]
            # anchor_model = 'adaface'
            fused_score_mat[i] = score_mats[anchor_model][i]
            fused_merge_score_mat[i] = merge_score_mats[anchor_model][i]
            for j in range(len(model_list)):
                if models_used_mask[j]:
                    model_name = model_list[j] # model_list sequence should be the same as model_mask
                    # Get the top-k values and their indices using numpy
                    topk_indices = np.argpartition(score_mats[model_name][i], -top_k)[-top_k:]
                    topk_values = normalize_score(score_mats[model_name][i], norm_method).copy()[topk_indices] * score_mats[model_name][i][topk_indices]
                    merge_topk_indices = np.argpartition(merge_score_mats[model_name][i], -top_k)[-top_k:]
                    merge_topk_values = normalize_score(merge_score_mats[model_name][i], norm_method).copy()[merge_topk_indices] * merge_score_mats[model_name][i][merge_topk_indices]
                    # Add the top-k values to the fused matrices
                    for k in range(top_k):
                        fused_score_mat[i, topk_indices[k]] += (topk_values[k])
                        fused_merge_score_mat[i, merge_topk_indices[k]] += (merge_topk_values[k])
            
            # Average the fused scores by the number of models used
            fused_score_mat[i] /= (num_models_used+1)
            fused_merge_score_mat[i] /= (num_models_used+1)  
            
    return fused_score_mat, fused_merge_score_mat


def act_score_fusion_gradio(score_mats, anchor_model_list, top_k=10, norm_method='zscore'):
    """
    Fuse scores from multiple models based on a model mask using proposed Anchor-based Confidence Top-k strategy (ACT).
    This is a simplified version for Gradio app, assuming all models are used for fusion.

    Args:
        score_mats (dict): A dictionary of score matrices from different models.
                           Keys are model names, values are numpy arrays of shape (num_probes, num_gallery).
        anchor_model_list (list): A list of anchor model names to initialize the fusion for each probe. Length must be num_probes.
        top_k (int): The number of top scores to use from each model for fusion.
        norm_method (str): The normalization method to apply to scores before fusion.
    ...
    num_probes, num_gallery = score_mats[model_list[0]].shape
    num_models = len(model_list)

    fused_score_mat = np.zeros((num_probes, num_gallery))

    for i in range(num_probes):
        # In this version, we assume all models are used.
        num_models_used = num_models
        
        anchor_model = anchor_model_list[i]
        fused_score_mat[i] = score_mats[anchor_model][i].copy()

        for model_name in model_list:
            scores_row = score_mats[model_name][i]
            
            # Get the top-k values and their indices using numpy
            topk_indices = np.argpartition(scores_row, -top_k)[-top_k:]
            
            topk_weighted_values = normalize_score(scores_row, norm_method)[topk_indices] * scores_row[topk_indices]

            # Add the top-k values to the fused matrices
            fused_score_mat[i, topk_indices] += topk_weighted_values

        # Average the fused scores by the number of models used + anchor
        fused_score_mat[i] /= (num_models_used + 1)
            
    return fused_score_mat


def evaluate_agent(similarity_matrix, q_pids, g_pids, far=[0.0001, 0.001, 0.01]):
    """ 
    Compute TAR@FAR (Verification), Ranking and FNIR@FPIR (Open-Search) metrics on Biometric recogntion task
    ...
    cmc_result = pd.Series([CMC[i] for i in cmc_index], index=[f"Rank{i+1}" for i in cmc_index])
    mAP_result = pd.Series([mAP], index=['mAP'])
    result = pd.concat([cmc_result, mAP_result, verification_result], axis=0)
    return result